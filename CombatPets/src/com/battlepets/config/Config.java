...
public abstract class Config {
	...
	public Config(String name, double version) {
		...
		this.base = BattlePets.getInstance();
	...
	public File backup() {
		...
		BattlePets.Log("Backing up...");
		
		if(backups.mkdirs() || backups.exists()) {

			BattlePets.Log("created");
			...
			if(!this.getConfig().exists()) return null;
				...
				if(success) {
					BattlePets.Log("&a" + this.name + ".yml has been backed up!");	
				}else {
					BattlePets.Log("&cCouldnt backup!");
				}
			
				return tempConfig;
			
		}else if(!backups.exists()) {
			BattlePets.Log("&cCouldnt create backup folder!");
		}
